# -*- coding: utf-8 -*-
"""
Created on Sat Oct 15 11:07:38 2022
...
###############################################################################
#
# Create a main data file for the 2016-17 season containing response and 
# explanatory variables. The file will include:
#     - Number of points scored by a player in a game
#     - Individual player statistics for previous games
#     - Cumulative player statistics for the season
#     - Team statistics
#     - Opposition statistics
#
#
# The returned file will only contain rows where players have appeared at least
# 4 times previously in the current season
# 
###############################################################################
...
def create_tabular_data(season_start, season_end, data_dir)
...
fpl_data_filename = data_dir + '/20{0}_{1}/20{0}_{1}_raw_data.csv'.format(season_start, season_end)
...
# Drop any redundant columns that will not be used in modelling. This includes
# administrative columns such as id and data from the match for which total_points
# are to be forecast such as assists etc.
#
# The column for minutes played will be retained to allow filtering players
# by this quantity.

redundant_cols = ['assists', 'attempted_passes', 'big_chances_created', 'big_chances_missed',
                  'bonus', 'bps', 'clean_sheets', 'clearances_blocks_interceptions',
                  ...
                  'id', 'influence', 'key_passes', 'kickoff_time', 'kickoff_time_formatted',
                  'loaned_in', 'loaned_out', 'offside', 'open_play_crosses',
                  'opp_adj_score', 'opp_nsxg', 'opp_score', 'opp_xg',
                  'opponent_team', 'own_goals', 'penalties_conceded', 'penalties_missed',
                  'penalties_saved', 'recoveries', 'red_cards', 'saves', 'tackled',
                  'tackles', 'target_missed', 'team_a_score', 'team_h_score',
                  'team_adj_score', 'team_nsxg', 'team_score', 'team_xg', 'threat',
                  'transfers_balance', 'value', 'winning_goals', 'yellow_cards',
                  'name', 'season_x', 'opp_team_name', 'kickoff_datetime', 'team_x']

data_main.drop(redundant_cols, axis=1, inplace=True)

#%%

#
# Clean up data ready for modelling
#
...
data_main_filename = (data_dir 
                     + '/20{0}_{1}/{0}_{1}_model_data.csv'.format(season_start, season_end))
